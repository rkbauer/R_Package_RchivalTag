...
abacus_plot <- plot_data_coverage <- function(x, type, type2, meta, 
  ...
  if(class(meta) != "data.frame") stop("meta data not of class 'data.frame'. Please revise!")
  ...
  if(missing(date_range_std)){
    ...
    if(show_fullmonths)
      ...
      date_range_std[1] <- paste0(substr(date_range_std[1],1,nchar(date_range_std[1])-2),"01")
      date_range_std[2] <- paste0(substr(date_range_std[2],1,5),"01")
